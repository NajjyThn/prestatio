...
const createProfessional = async (req, res) => {
  const { specialty, city, address, bio } = req.body;
  const user_id = req.user.id;

  try {
    // Vérifier que l'user n'a pas déjà un profil pro
    ...
    const result = await pool.query(`
      INSERT INTO professionals (user_id, specialty, city, address, bio)
      VALUES ($1, $2, $3, $4, $5)
      RETURNING *
    `, [user_id, specialty, city, address, bio]);
...
const updateProfessional = async (req, res) => {
  ...
  try {
    const result = await pool.query(`
      ...
      SET specialty = $1, city = $2, address = $3, bio = $4
      WHERE user_id = $5
      RETURNING *
    `, [specialty, city, address, bio, user_id]);
